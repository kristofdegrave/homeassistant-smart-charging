...
def test_effective_peak_limit_is_the_lesser_of_monthly_peak_and_maximum():
    ...
    assert (
        resolve_effective_peak_limit(monthly_peak_kw=3.0, max_peak_kw=4.0, peak_floor_kw=0.0, urgent=False)
        == 3.0
    )
    assert (
        resolve_effective_peak_limit(monthly_peak_kw=5.0, max_peak_kw=4.0, peak_floor_kw=0.0, urgent=False)
        == 4.0
    )
...
def test_urgency_raises_to_the_maximum_peak_regardless_of_monthly_peak():
    assert (
        resolve_effective_peak_limit(monthly_peak_kw=1.0, max_peak_kw=4.0, peak_floor_kw=0.0, urgent=True)
        == 4.0
    )
...
def test_urgency_never_exceeds_the_maximum_peak():
    ...
    assert (
        resolve_effective_peak_limit(monthly_peak_kw=10.0, max_peak_kw=4.0, peak_floor_kw=0.0, urgent=True)
        == 4.0
    )
...
def test_low_monthly_peak_is_raised_to_the_floor():
    ...
    assert (
        resolve_effective_peak_limit(monthly_peak_kw=0.5, max_peak_kw=4.0, peak_floor_kw=2.5, urgent=False)
        == 2.5
    )
...
def test_monthly_peak_above_the_floor_is_unaffected_by_it():
    ...
    assert (
        resolve_effective_peak_limit(monthly_peak_kw=3.0, max_peak_kw=4.0, peak_floor_kw=2.5, urgent=False)
        == 3.0
    )
...
def test_floor_never_raises_the_limit_above_the_maximum_peak():
    ...
    assert (
        resolve_effective_peak_limit(monthly_peak_kw=0.5, max_peak_kw=4.0, peak_floor_kw=10.0, urgent=False)
        == 4.0
    )
...
def test_urgency_ignores_the_peak_floor_too():
    ...
    assert (
        resolve_effective_peak_limit(monthly_peak_kw=0.1, max_peak_kw=4.0, peak_floor_kw=2.5, urgent=True)
        == 4.0
    )
