...
def test_dashboard_has_two_views_overview_and_deadline():
    """T9: HA renders >1 views in a YAML dashboard as tabs natively --
...
def test_runtime_settings_section_has_the_mode_gate_and_the_auto_entities_card():
    ...
    # T8: the mode selector only makes sense under the Manual profile
    ...
    # different, legitimate ones: mode is rendered by the conditional card above instead (T8),
    # and the nine departure-time entities move to the deadline tab instead (T9) -- neither
    # should duplicate here.
