...
chain at that step. Only the **Clean up** step is dispatched by no chain step: it starts from the
human stating that the merge happened. The rules behind both are that doc's **Rule B** and
**Rounds and the cap** sections.
