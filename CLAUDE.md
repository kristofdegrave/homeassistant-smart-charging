...
| Topic | Owner |
|---|---|
| **Idea-to-product flow** | [idea-to-product.md](docs/reference/idea-to-product.md) — the method's default flow: twelve stages from capturing an idea to closing its epic, each naming its artifact, the gate before the next and the skills used at it; brainstorming is mandatory; a stage whose work type the profile does not enable is skipped; this project's deviations are `profile.md`'s **Flow**. |
...
| **Writing order** | [idea-to-product.md#6-analysis](docs/reference/idea-to-product.md#6-analysis) — the *Analysis first, in this order* rule of the flow's **Analysis** stage; the design documents follow at its **Design** stage. |
| **Requirements standard** | [idea-to-product.md#6-analysis](docs/reference/idea-to-product.md#6-analysis) — the *What, never how* rule of the **Analysis** stage: MoSCoW, SMART, the 6Cs. |
| **DDD alignment (lightweight)** | [idea-to-product.md#6-analysis](docs/reference/idea-to-product.md#6-analysis) — the *Two DDD concepts adopted* rule of the **Analysis** stage: glossary-first, domain events. |
| **Review protocol for analysis documents** | [idea-to-product.md#6-analysis](docs/reference/idea-to-product.md#6-analysis) — the **Analysis** stage's last three rules: the `uc`/`requirement` rows own draft and review; no tracking refs in a document body, ADRs included; the `specs`-issue gate on `needs-approval`. |
| **Flow document standard** | [idea-to-product.md#6-analysis](docs/reference/idea-to-product.md#6-analysis) — the *Section order and Mermaid types* rule of the **Analysis** stage. |
