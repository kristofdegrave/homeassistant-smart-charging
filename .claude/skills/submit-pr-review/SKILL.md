...
- **Locally**: the `review` skill runs the pass. Every reviewer it spawns is read-only —
  ...
  One pass is one review, however many reviewers ran. The PR always exists by then: step 1
  of the contribution workflow — the doc `CLAUDE.md`'s **Contribution workflow** section
  routes to — guarantees it.
