...
- **Locally**: the `review` skill runs the pass. Every reviewer agent the work-types table
  names is read-only — they return findings, they do not post — so after each returns, the
