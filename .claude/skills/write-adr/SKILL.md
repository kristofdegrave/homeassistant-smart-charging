...
- **Step 3's review** — reviewer per `CLAUDE.md`'s **Model selection** table, the `adr` row.
  It checks template conformance, that every option has a genuine Pro and Con, that the
  Decision references those trade-offs, that Consequences actually follow, and cross-ADR
  consistency (including the immutability rule). Not `analysis-reviewer` — that agent is
  scoped to `docs/analysis/**` and doesn't cover `docs/adl/**`.
