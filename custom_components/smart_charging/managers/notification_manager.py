...
class NotificationManager:
    ...
    - `_state`/`_date` are in-memory only and start afresh on every HA restart and reload, so
      ...
      can cause a second prompt the same evening (UC08's "at most once per evening" is only
      guaranteed within one HA session). NF14's own list of what a restart and a reload clear
      names "each notification's record of having been sent" explicitly; this Manager's
      prompt-sent record is that choice applied here, not something still owed to it.
    """
