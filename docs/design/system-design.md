...
| 0031 | Config-entry values also exposed as disabled-by-default diagnostic sensors | **Adds read-only mirrors; consistent** | One read-only diagnostic sensor, disabled by default, per `config-options` value except `control_interval_s`, which the ADR carves out, per boolean capability in `config-data`, each read from the config entry — the Store's two buckets (V13). They are mirrors, never a second place a value can be set, so ADR-0005's placement stands and the owned population ADR-0004 draws is unchanged. |
