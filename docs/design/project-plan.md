...
- **Integration checkpoint:** ⎔ Coordinator (M1) and the Clients read config/options and (post-gate)
  owned entities through the Store, with no direct HA-registry access outside it.
...
- **Builds:** the ordered cycle from [system-design §5.1](system-design.md#51-control-cycle-realizes-uc01uc04-and-uc05uc07-in-passing):
  ...
  diagnostics (`sensor.sc_monthly_peak_kw`, Fault/OK) through the Store (RA3). Realizes UC01–UC04 and
...
- **Depends on:** RA3 (vehicle_charge_limit adapter — see RA note below), RA1 (`car_home`,
  `charger_status`), RA3 Store (write `sc_active_soc`), E3 (SOC-Target); its triggers are
...
- **Integration checkpoint:** ⎔ subscribes per ADR-0011; writes `vehicle_charge_limit` and
  `sc_active_soc` through adapter/Store; no direct call to/from M1.
