...
| `low_tariff` | adapter role | — | bool | mapped to the installation's tariff signal, with a user-supplied list of raw states that count as low tariff when the mapped entity's own state is not already boolean (NF3; optional — treated as always `on` when not configured — single-tariff installation) | [low-tariff flag](system-overview.md#ubiquitous-language) | resolution-rules | — |
