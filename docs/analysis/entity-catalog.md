...
| `charger_power` | adapter role | — | W | mapped to the charger's power sensor (NF3); normalised to W at the adapter, converting a source entity that reports another power unit and assuming W when it reports none | charger power (operand of [solar surplus](system-overview.md#ubiquitous-language)) | control-cycle, UC01, UC02 | — |
...
| `solar_power` | adapter role | — | W | mapped to the installation's solar production sensor (NF3); normalised to W at the adapter, converting a source entity that reports another power unit and assuming W when it reports none | solar production reading, read raw and never smoothed (R10); not an operand of [solar surplus](system-overview.md#ubiquitous-language), which is `charger_w − net_w` | control-cycle | — |
