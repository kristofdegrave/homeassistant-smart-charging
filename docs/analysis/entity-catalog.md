...
sole place to *set* it; the sensor only lets a household *see* it without opening Configure. Two
runtime helper values remain an open question under ADR-0004 and
...
| `sensor.smart_charging_charger_status` | state | — | — | the same per-cycle reading as `adapter_readings`'s `charger_status` attribute: `disconnected`/`connected`/`charging` — an unmapped-but-present raw state now resolves to `disconnected` (ADR-0035), not unknown; unknown is reserved for a missing or unavailable mapped entity (ADR-0007 semantics); per ADR-0034 | [charger status](system-overview.md#ubiquitous-language) (`disconnected`/`connected`/`charging`) | UC11 | control-cycle |
