...
- **Setup** — for a `config` or `state` row the user sets directly, whether it is
  ...
  [ADR-0031](../adl/0031-config-values-as-disabled-by-default-diagnostic-sensors.md), every such row
  except `control_interval_s` also carries a sibling `state` row — a read-only, disabled-by-default
  sensor mirror (see Notes) — purely for in-UI visibility; that mirror is itself never presented on
  the runtime dashboard either (an ADR-0031/[ADR-0022](../adl/0022-runtime-dashboard-delivery-mechanism.md)
  design choice, not an R19 acceptance criterion — R19 governs runtime/install-time entities, and a
  mirror is neither), and its existence does not change this row's own `Setup`/`Role` classification.
  `—` marks `adapter role` rows (a code-level mapping, not a catalogued entity) and `state` rows that
...
| `sensor.smart_charging_solar_available` | state | — | — | mirrors `solar_available` (config-data); disabled by default (ADR-0031) | [capability](system-overview.md#ubiquitous-language) — solar (R18) | user | — |
| `sensor.smart_charging_captar_available` | state | — | — | mirrors `captar_available` (config-data); disabled by default (ADR-0031) | [capability](system-overview.md#ubiquitous-language) — CapTar (R18) | user | — |
| `sensor.smart_charging_deadline_available` | state | — | — | mirrors `deadline_available` (config-data); disabled by default (ADR-0031) | [deadline capability](system-overview.md#ubiquitous-language) (R18) | user | — |
| `sensor.smart_charging_notifications_available` | state | — | — | mirrors `notifications_available` (config-data); disabled by default (ADR-0031) | [notifications capability](system-overview.md#ubiquitous-language) (R18) | user | — |
...
| `sensor.smart_charging_safety_margin_w` | state | — | W | mirrors `safety_margin_w` (config-options); disabled by default (ADR-0031) | [safety margin](system-overview.md#ubiquitous-language) | user | — |
...
| `sensor.smart_charging_max_peak_kw` | state | — | kW | mirrors `max_peak_kw` (config-options); disabled by default (ADR-0031) | [maximum peak](system-overview.md#ubiquitous-language) | user | — |
...
| Id | Role | Setup | Unit | Default / range / source | Realizes | Read by | Written by |
| --- | --- | --- | --- | --- | --- | --- | --- |
| `number.smart_charging_target_current` | config | runtime | A | 10 (min–max charging current) | [Power target current](system-overview.md#ubiquitous-language) (R17) | UC04, UC11 | user, UC11, UC12 (seeds initial value) |
...
| `sensor.smart_charging_power_respect_peak` | state | — | — | mirrors `power_respect_peak` (config-options); disabled by default (ADR-0031) | `Power` peak-protection option (R17) | user | — |
| `power_cooldown_min` | config-options | options | min | 10 | `Power`-mode cooldown (R11) | UC04 | user (anytime), UC12 |
| `sensor.smart_charging_power_cooldown_min` | state | — | min | mirrors `power_cooldown_min` (config-options); disabled by default (ADR-0031) | `Power`-mode cooldown (R11) | user | — |
...
| `sensor.smart_charging_solar_surplus_w` | state | — | W | `charger_power − net_power`, floored at 0, computed fresh each control cycle, never stored; reads the same accepted [household baseline](system-overview.md#ubiquitous-language) the R3 clamp does (negated), so on a cycle R3 defers that reading this value holds its previously displayed one rather than showing the deferred reading; gated on the solar capability (R18) — registry-disabled while `solar_available` is off (ADR-0028), so the dashboard omits it (R19 AC4, UC11 3a) | [solar surplus](system-overview.md#ubiquitous-language) | UC11 | control-cycle |
...
| `sensor.smart_charging_charger_status` | state | — | — | the same per-cycle reading as `adapter_readings`'s `charger_status` attribute: `disconnected`/`connected`/`charging`; an unmapped-but-present raw state resolves to `disconnected` (ADR-0035), while `unknown` is reserved for a missing or unavailable mapped entity (ADR-0007 semantics); per ADR-0034 | [charger status](system-overview.md#ubiquitous-language) (`disconnected`/`connected`/`charging`) | UC11 | control-cycle |
...
| `sensor.smart_charging_solar_start_threshold_w` | state | — | W | mirrors `solar_start_threshold_w` (config-options); disabled by default (ADR-0031) | [solar start threshold](system-overview.md#ubiquitous-language) (R1) | user | — |
...
| `sensor.smart_charging_solar_hold_min` | state | — | min | mirrors `solar_hold_min` (config-options); disabled by default (ADR-0031) | [post-surplus hold](system-overview.md#ubiquitous-language) (R1) | user | — |
...
| `solar_power` | adapter role | — | W | mapped to the installation's solar production sensor (NF3); normalised to W at the adapter, converting a source entity that reports another power unit and assuming W when it reports none. A unit that is present but is not a convertible power unit reads as absent, with no effect on the control cycle beyond the role being unavailable. | solar production reading, read raw and never smoothed (R10); not an operand of [solar surplus](system-overview.md#ubiquitous-language), which is `charger_w − net_w` | control-cycle | — |
...
| `sensor.smart_charging_solar_only_start_threshold_w` | state | — | W | mirrors `solar_only_start_threshold_w` (config-options); disabled by default (ADR-0031) | [solar start threshold](system-overview.md#ubiquitous-language) — SolarOnly instance (R2) | user | — |
...
| `sensor.smart_charging_solar_only_hold_min` | state | — | min | mirrors `solar_only_hold_min` (config-options); disabled by default (ADR-0031) | [post-surplus hold](system-overview.md#ubiquitous-language) — SolarOnly instance (R2, R11) | user | — |
...
| `sensor.smart_charging_solar_only_rounding_strategy` | state | — | — | mirrors `solar_only_rounding_strategy` (config-options); disabled by default (ADR-0031) | [amp-step rounding](system-overview.md#ubiquitous-language) strategy (R2) | user | — |
...
| `sensor.smart_charging_solar_only_rounding_midpoint_pct` | state | — | % | mirrors `solar_only_rounding_midpoint_pct` (config-options); disabled by default (ADR-0031) | [amp-step rounding](system-overview.md#ubiquitous-language) midpoint — `nearest` strategy only (R2) | user | — |
...
| `sensor.smart_charging_max_solar_soc` | state | — | % | mirrors `max_solar_soc` (config-options); disabled by default (ADR-0031) | [solar step-up](system-overview.md#ubiquitous-language) ceiling (R8) | user | — |
| `solar_step_pp` | config-options | options | pp | 5 | solar step-up size (R8) | UC06 | user (anytime), UC12 |
| `sensor.smart_charging_solar_step_pp` | state | — | pp | mirrors `solar_step_pp` (config-options); disabled by default (ADR-0031) | solar step-up size (R8) | user | — |
| `solar_step_threshold_pp` | config-options | options | pp | 2 | solar step-up trigger gap (R8) | UC06 | user (anytime), UC12 |
| `sensor.smart_charging_solar_step_threshold_pp` | state | — | pp | mirrors `solar_step_threshold_pp` (config-options); disabled by default (ADR-0031) | solar step-up trigger gap (R8) | user | — |
...
| `sensor.smart_charging_deadline_notice_enabled` | state | — | — | mirrors `deadline_notice_enabled` (config-options); disabled by default (ADR-0031) | unreachable-deadline notice enable (R5, R18) | user | — |
| `plug_in_reminder_enabled` | config-options | options | — | on | plug-in reminder enable (R12, R18) | UC10 | user (anytime), UC12 |
| `sensor.smart_charging_plug_in_reminder_enabled` | state | — | — | mirrors `plug_in_reminder_enabled` (config-options); disabled by default (ADR-0031) | plug-in reminder enable (R12, R18) | user | — |
| `evening_prompt_enabled` | config-options | options | — | on | evening home-day prompt enable (R13, R18) | UC08 | user (anytime), UC12 |
| `sensor.smart_charging_evening_prompt_enabled` | state | — | — | mirrors `evening_prompt_enabled` (config-options); disabled by default (ADR-0031) | evening home-day prompt enable (R13, R18) | user | — |
| `evening_prompt_time` | config-options | options | time | 18:00 | evening prompt time (UC08) | UC08 | user (anytime), UC12 |
| `sensor.smart_charging_evening_prompt_time` | state | — | time | mirrors `evening_prompt_time` (config-options); disabled by default (ADR-0031) | evening prompt time (UC08) | user | — |
...
- **Runtime target vs. install-time/config-entry bound — judgment calls.** Where a value isn't a
  ...
  **ceiling/bound** on top of a target (`max_solar_soc`, a step-up ceiling, not
  ...
  (`number.smart_charging_target_current`). For values ADR-0005's own text does not individually
  enumerate (e.g. `grid_supply_ceiling_a`,
...
- **`sensor.smart_charging_adapter_readings`** (ADR-0021, R19) mirrors every currently-wired
  *read* adapter role's current value as an attribute, giving the dashboard something to bind to
  ...
  asks for instead.
- **`sensor.smart_charging_charger_status`** (ADR-0034, extending ADR-0021's single-role exception
  clause) is one of three distinct things this catalog names similarly, and none of the three is a
  copy of another:
  - the `charger_status` **adapter role** row (Charger, above) — the mapping *input*: the raw
    upstream entity plus the user-supplied translation table, not an HA entity at all;
  - `sensor.smart_charging_status` (Diagnostic outputs, above, ADR-0007) — the integration's own
    health readout (`OK`/`Fault`), unrelated to the charger's connection state;
  - this row — the mapping's *output*, materialized as its own owned entity so the value can be
    historised and triggered on, fed from the same per-cycle reading `adapter_readings`'s
...
- **Config-value mirror sensors ([ADR-0031](../adl/0031-config-values-as-disabled-by-default-diagnostic-sensors.md)).**
  Every `config-options` row except `control_interval_s` (excluded per ADR-0031's own scope, honoring
  [ADR-0005](../adl/0005-config-entry-structure-and-interval.md)'s "not an owned entity" wording for
  that one value), plus the four capability booleans under *Capabilities*, now has a sibling `state`
  ...
  *Peak protection* (that row's `Written by` is `control-cycle`, since unlike these mirrors it is
  ...
  row — and their existence does not change the source row's own `Setup`/`Role` classification: the
  config entry remains the sole place to *set* the value. `control_interval_s` and the `adapter role`
  rows (entity-role mappings, state-translation tables — never `config-data`/`config-options` rows
  themselves) have no mirror sensor (ADR-0031's scope).
