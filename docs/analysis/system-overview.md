...
**`profile`** — An extensible, higher-level strategy that determines which mode is active over time. A profile is defined by three independent behaviours: (1) **mode selection** — how the active mode is chosen from observable conditions, (2) **reserve coordination** — whether it declines to select a mode for opportunistic overnight top-up when the solar-reserve cap (R9) is in effect, and (3) **escalation levers** — which modes, if any, departure-deadline urgency (R5) may escalate to. This release ships two built-in profiles, each a fixed combination of the three: `Manual` (the user selects the active mode directly; never declines top-up; no escalation levers) and `Auto` (selects from `resolution-rules.md`; coordinates the reserve cap per R9; escalates to `Captar`/`Power` per R16). The concept is deliberately designed so additional profiles can be added later — recombining the three behaviours differently — and, in future, so users could define their own profiles with custom behaviour. A profile *sets* the active mode; it is not itself a mode. Selected via `select.smart_charging_profile`. NF1 holds: profiles decide the mode, the coordinator only executes it.
...
- **User-defined custom profiles** — letting users author their own profiles by recombining the profile concept's three behaviours (mode selection, reserve coordination, escalation levers) is a future capability. The mode/profile model is designed to make it possible, but no authoring mechanism is provided this release.
