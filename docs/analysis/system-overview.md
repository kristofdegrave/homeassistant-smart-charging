...
**`capability`** — An installation characteristic, declared by configuration, that determines which modes and behaviours are available. This release recognises two: the **solar capability** (a solar PV installation is present, `input_boolean.sc_solar_available`, default present) and the **CapTar capability** (the installation bills against a capacity tariff, `input_boolean.sc_captar_available`, default present). When the solar capability is absent, the `Solar` and `SolarOnly` modes, the solar SOC step-up, and the solar-reserve cap are all unavailable, and the `Auto` profile never selects a solar mode. When the CapTar capability is absent, the `Captar` mode is unavailable for manual selection and is never chosen by the `Auto` profile. `Power` and `Off` remain available regardless of either capability. The concept is extensible so further capabilities (e.g. a home battery) can be added later, each gating the modes and behaviours that depend on that hardware, without changing existing modes (NF2). See R18.
