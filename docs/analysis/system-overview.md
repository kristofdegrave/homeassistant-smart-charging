...
**`post-surplus hold`** — In `Solar` mode, the period (configurable, `solar_hold_min`, default 5 minutes) for which the charger holds at the minimum charging current after smoothed solar surplus falls below the solar start threshold, riding out brief cloud cover before charging stops; if surplus recovers to the start threshold within the period the hold is cancelled. `SolarOnly` has no such hold — it stops immediately. See R1. Unit: minutes (min).
