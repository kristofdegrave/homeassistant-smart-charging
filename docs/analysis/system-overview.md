...
**`time to full charge`** — An estimate of how long charging would take, from now, to reach the active SOC limit at the charger's current applied rate — computed from the EV battery capacity (R15), current state of charge, the active SOC limit, and the charger's current set-point (`charger_current`). A dashboard-only readout (R19); unlike [required current](#ubiquitous-language) it does not drive any control decision and carries no urgency semantics — a slow current simply yields a longer estimate, never a deadline escalation. Unit: minutes (min).
