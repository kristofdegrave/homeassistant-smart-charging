...
- [ ] The unreachable-deadline notice is sent only while the notifications capability is present **and** its own enable toggle (`deadline_notice_enabled`, default on) is on (R18). When that toggle is off, the notice is never sent even though the capability is present and a notification target is mapped. Charging behaviour is unaffected — the system still charges at the maximum permitted rate — and so is the occasion bookkeeping below: occasions still begin, end, and re-arm while the toggle is off, so turning it back on releases no backlog of notices for occasions that have already passed.
...
- [ ] The evening home-day prompt — the mechanism by which the system asks for the flag rather than waiting to be told — is a notification like R5's notice and R12's reminder, and carries the same kind of per-notification enable toggle: it is sent only while the notifications capability is present **and** its own toggle (`evening_prompt_enabled`, default on) is on (R18). The two readings of that toggle coincide, since the prompt *is* the notification: turning it off stops the prompt being sent and, by the same act, withdraws it as a mechanism for setting the flag. Nothing else is withdrawn — every other configured mechanism (a manual input, an external calendar or presence source, NF3) still sets the flag, and if none remains configured the flag is simply never set and tomorrow is treated as not a home day (below).
