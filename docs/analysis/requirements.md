...
- [ ] When even charging at the maximum permitted rate cannot meet the deadline, the system charges at that maximum and sends the user a notification that the deadline is unreachable. This notification — and the per-occasion criteria below that govern when it is sent and re-armed — additionally depends on the notifications capability being present (R18 AC10); when that capability is absent, the notice is not sent at all. Only this notification behaviour is scoped that way: R5's charging behaviour is unaffected either way — the peak-limit raise, `Auto`'s escalation, and the SOC-limit protections above all still apply, and the system still charges at the maximum permitted rate.
...
- [ ] A single notification is sent when the car is home, disconnected, below the active SOC limit, and within a configurable lead time (default 8 hours) of the next departure time (R14). The reminder additionally depends on the notifications capability being present (R18 AC10); when that capability is absent it is not sent at all. R12 describes no behaviour other than the reminder itself, so nothing of this requirement remains in force in that case.
...
- [ ] The home-day flag for tomorrow can be set through at least one configured mechanism, whether a system-provided input or an external source (NF3). A notification prompt (the evening home-day prompt) is one such mechanism only while the notifications capability is present (R18 AC10); when that capability is absent the prompt is not sent at all, and only the prompt's delivery falls away — the home-day flag's own mechanism still accepts its other configured inputs, and every criterion below applies unchanged.
