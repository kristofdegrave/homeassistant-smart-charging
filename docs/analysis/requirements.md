...
- [ ] A home-day flag set through the system's own mechanisms — its manual home-day input or the evening prompt — is set for one calendar date, tomorrow's at the moment it is set, and applies to that date alone, until that date ends at midnight. For the whole of that date R14 resolves its departure time with the home-day override, so the departure-deadline resolution moves past the home day exactly as it would past any other date and never falls back to that date's day-of-week default. No separate reset is needed. The manual home-day input shows and sets tomorrow's flag; a flag already applying to today is fixed once its date has begun.
