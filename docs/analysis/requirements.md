...
- [ ] A capability change never overrides the user's own choice to enable or disable one of the system's entities. An entity the user has disabled stays disabled, and one the user has enabled while a capability that gates it is absent stays enabled — through every later capability change, restart and reload — until the user changes it again. Only an entity whose enabled state the user has left alone follows the capabilities: disabled while a capability that gates it is absent, and back to enabled, its default, once that capability is present (ADR-0028).
