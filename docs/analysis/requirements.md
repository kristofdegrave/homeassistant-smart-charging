...
- [ ] The home-day flag is set for one calendar date — tomorrow's, at the moment it is set — and applies to that date alone, for the solar-reserve cap (R9) and the departure-time resolution (R14) alike. It stops applying once the departure-deadline resolution (R14) has moved past that date — at that date's end, the same moment whether or not the deadline capability is present — so no separate reset is needed, and setting the flag for tomorrow never changes a flag already applying to today.
...
- [ ] The home-day flag survives a restart and a reload still bound to the date it was set for (R13): afterwards it applies to that date exactly as it would have without one, and it no longer applies once the departure-deadline resolution has moved past that date — however long the system was stopped.
