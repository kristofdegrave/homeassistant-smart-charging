...
- [ ] The smoothed baseline that forecast is fitted to is the negation of R10's smoothed [solar surplus](system-overview.md#ubiquitous-language): the mean, over the samples R10's window has admitted, of each control cycle's net import minus that same cycle's charger power. It carries **no** R3 deferral. R3's two cases exist to keep a per-instant clamp from acting on the system's own actuation or on a single optimistic headroom increase, and their per-case one-cycle bound is written for that clamp's safety rather than for a forecast. R10's window keeps the first out of the forecast: pairing each sample with its own cycle's charger power leaves none of the charger's earlier draw in the mean, and R10's admission rule leaves out the sample a lagging charger power reading would spoil on the cycle after the charger current changes. It dilutes the second to one sample of *N*, which is all a forecast needs. So, at any window above 1, once the window holds at least one sample and every sample it holds was taken under R10's steady inputs — none of them on a cycle R10 admitted only because it never leaves out two in a row — a change of charger current with no other change on the control cycle before or after it leaves the escalated maximum permitted rate unchanged on every control cycle.
