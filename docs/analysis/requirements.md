...
- [ ] This requirement applies only while the CapTar capability is present (R18). When it is absent, no peak-protection clamp ever engages: net grid import is bounded only by the grid supply ceiling (C4), and every criterion below is vacuous.
...
- [ ] The two cases compose to at most 3 consecutive control cycles of deferral, and only for a reading that would increase available headroom.
