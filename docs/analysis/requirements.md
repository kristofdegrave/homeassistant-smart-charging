...
- [ ] The smoothed baseline that forecast is fitted to is the negation of R10's smoothed [solar surplus](system-overview.md#ubiquitous-language): the mean, over the samples R10's window has admitted, of each control cycle's net import minus that same cycle's charger power. It carries **no** R3 deferral. R3's two cases exist to keep a per-instant clamp from acting on the system's own actuation or on a single optimistic headroom increase, and their per-case one-cycle bound is written for that clamp's safety rather than for a forecast. R10's window keeps the first out of the forecast: pairing each sample with its own cycle's charger power leaves none of the charger's earlier draw in the mean, and R10's first criterion leaves out the sample taken on the cycle after the charger current changes, which a draw or power reading still lagging that change would spoil, except in the cases that criterion admits it anyway. It dilutes the second to one of the samples the window holds, which is all a forecast needs. So, at a window above 1, within one unbroken spell of R10's steady inputs: a charger current the system sets that differs from the one it set on the control cycle before, with no other such change on the control cycle before or after it, leaves the escalated maximum permitted rate unchanged — and so, on its own, neither engages urgency nor makes the deadline unreachable — on every control cycle of that spell at which every sample the window holds was taken within the spell, none of them on a cycle R10 admitted only because it never leaves out two in a row.
