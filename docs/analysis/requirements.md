...
- [ ] Switching the active mode resets the hold and restart-debounce timers so the incoming mode starts fresh against its own thresholds — neither reset can bring a start forward, so neither can cause rapid cycling: a hold is a period spent *charging* at the minimum current, so ending it early stops nothing, and resetting a debounce only delays the next start. A cooldown already running is the exception and is **not** reset, per the criterion above. The has-charged flag is likewise unaffected, since it is scoped to the connection, not the active mode.
