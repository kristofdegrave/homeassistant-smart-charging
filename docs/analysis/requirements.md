...
- [ ] When the notifications capability is absent, the notification configuration surface is neither offered nor required: the notification-target mapping, the evening home-day prompt's enable flag and time, and the prompt timeout (R20, [UC12](use-cases/UC12-configure-installation-through-guided-flow.md)). Nothing beyond that surface falls away — the capability gates no charging mode and no clamp, the home-day flag's own mechanism (R13) still accepts its other configured inputs, and the plug-in reminder's lead time (R12) stays tied to the deadline capability rather than to this one.
