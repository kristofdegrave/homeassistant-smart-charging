...
- [ ] The smoothed baseline that forecast is fitted to is the one R10's smoothing window forms: the mean of the samples that window has admitted, each pairing one control cycle's net import with that same cycle's charger power — negated, the smoothed [solar surplus](system-overview.md#ubiquitous-language). It carries **no** R3 deferral. R3's two cases exist to keep a per-instant clamp from acting on the system's own actuation or on a single optimistic headroom increase, and their per-case one-cycle bound is written for that clamp's safety rather than for a forecast. R10's window already keeps both out of the forecast: pairing each sample with its own cycle's charger power leaves none of the charger's earlier draw in the mean, R10's admission rule leaves out the sample a lagging charger power reading would spoil on the cycle after the charger current changes, and a single optimistic reading is one sample of *N*. So, under R10's steady inputs and at any window above 1, a change of charger current that does not immediately follow another leaves the escalated maximum permitted rate unchanged on every control cycle.
