...
- [ ] The cap's three date-bearing conditions — the home-day flag, the solar forecast and the departure-deadline resolution — all read the reserved day: tomorrow's date until midnight, and the date that has just begun from midnight until the sun comes up. None of them changes the date it reads at midnight, so midnight alone never engages or lifts the cap.
- [ ] The solar forecast for the reserved day is read from two configured forecast sensors (NF3): a next-day forecast until midnight, and a same-day forecast from midnight until the sun comes up. Two are needed because the forecast sources an installation maps report a yield per calendar date, so once the date rolls over a next-day sensor reports the day after the reserved day ([research](https://github.com/kristofdegrave/homeassistant-smart-charging/issues/1362#issuecomment-5837468595)). Both must be mapped whenever the solar capability is present (R18), and neither is a required role in C5's sense.
