...
- [ ] Once engaged, urgency stays in effect until one of these clears it: the [baseline mode](system-overview.md#ubiquitous-language)'s own desired current reaches the required current *on a cycle at which the engage condition above does not itself hold* (the ordinary policy will now meet the deadline unaided — e.g. the low tariff has opened and `Auto`'s own overnight row would charge anyway). The engage condition takes precedence where both hold at once, which it can, since a mode's desired current is what it asks for rather than what can be delivered; without that precedence the two conditions would alternate cycle by cycle. The comparison is against the baseline mode — the one that would be active absent the escalation — never the escalated mode itself, whose request is always the maximum and would clear urgency the instant it engaged. Urgency also clears when a missed-deadline hold ends (below); state of charge reaches the active SOC limit; the car is disconnected; the departure deadline resolves to "no deadline"; or the deadline capability becomes absent (R18). It is **not** re-derived from the engage condition above each cycle: charging at the escalated rate closes the gap faster than the deadline approaches, so re-deriving it would revert urgency on the cycle after it engaged and cycle the charger on and off.
