...
- [ ] Under steady inputs the charger set-point settles and then holds, in every mode. *Steady inputs* means the household's own consumption, solar production, the supply voltage, the low-tariff flag, the effective peak limit, the active mode and every configured value stay unchanged, and the charger's draw and its power reading both reach the current it was last set to within one control cycle. From the (N + 3)th control cycle after the last change in any of them — N being this smoothing window, so the 7th at the default of 4 — the charger current is the same on every control cycle for as long as they stay unchanged. Only three things may still change it: a hold, peak-breach grace period, cooldown or restart debounce elapsing (R1, R2, R3, R11); the active SOC limit being reached (R6, R7) or deadline urgency engaging or ending (R5), as the car charges and time passes; and the grid supply ceiling clamp (C4) while it binds, which acts on each cycle's own raw readings (R3). R3's cap on deferring the household baseline bounds how long a reading is held back, not whether the set-point settles, so it does not stand in for this criterion.
...
- [ ] A restart of Home Assistant or a reload of the system — a reload being how a change saved through the [configuration flow](system-overview.md#ubiquitous-language) takes effect (NF11, ADR-0008) — ends every running post-surplus hold, peak-breach grace period, cooldown and restart debounce: none carries over, and none resumes or starts over (NF14). The first control cycle after either judges the active mode's own conditions afresh, as on a fresh connection: charging starts as soon as the mode's start condition holds, with no cooldown or restart debounce left to wait out; where it does not hold, the charger is at 0 A from that cycle, without first holding at the minimum charging current; and a peak breach that persists is timed from that cycle as a new one (R3). The trade-off is deliberate: both events are rare, and a reload is the household reconfiguring the system, whereas the protection this requirement gives is against the system's own normal operation cycling the charger (ADR-0008).
...
- [ ] A capability change never overrides the user's own choice to enable or disable one of the system's entities. An entity the user has disabled stays disabled, and one the user has enabled while a capability that gates it is absent stays enabled — through every later capability change, restart and reload — until the user changes it again. Only an entity whose enabled state the user has left alone follows the capabilities: disabled while a capability that gates it is absent, enabled again once that capability is present (ADR-0028).
