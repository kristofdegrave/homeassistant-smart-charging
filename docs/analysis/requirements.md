...
- [ ] This requirement applies only while the CapTar capability is present (R18). When it is absent, no peak-protection clamp ever engages: net grid import is bounded only by the grid supply ceiling (C4), and every criterion below is vacuous — except the [household baseline](system-overview.md#ubiquitous-language) acceptance criteria below, from the one naming the two deferral cases through the grid-supply-ceiling exclusion, which hold regardless of capability. That resolution runs every control cycle (`control-cycle.md` step 1), not inside the clamp this requirement gates, because readouts gated on other capabilities consume its result — `sensor.smart_charging_solar_surplus_w` on a solar installation without CapTar being the case that forces it.
...
- [ ] This check uses raw (unsmoothed) sensor readings and never the smoothed solar surplus of R10, so that a breach cannot persist for the duration of a smoothing window.
- [ ] The [household baseline](system-overview.md#ubiquitous-language) this check solves around is this control cycle's own reading, except in exactly two cases, in which the most recently accepted reading stands instead: (a) the charger current the System set on the previous control cycle (`ChargerCurrentSet`) differed from the one it set before that, so the reading partly measures the System's own actuation rather than the household — and never on two consecutive cycles, case (a) not applying at all on a control cycle immediately following one it deferred; (b) the reading would *increase* available headroom and is the first observation to do so — the second consecutive observation that still increases headroom is accepted, so case (b) acting alone defers exactly one control cycle.
- [ ] A case-(a) deferral is not an observation of the household, so it neither advances nor resets case (b)'s count: a reading part-way through a case-(b) sequence resumes where it left off, and the two cases may interleave in either order.
- [ ] No run of consecutive deferrals exceeds 3 control cycles. That worst case needs case (a) to apply on alternate cycles throughout a case-(b) sequence; either case acting alone defers exactly one.
- [ ] How often such a run may recur is deliberately not bounded: a household reading that keeps rising and falling is deferred again each time it falls, one cycle at a time, and the value that stands meanwhile is always the more conservative of the two.
- [ ] A household increase large enough to breach the effective peak limit minus the safety margin is deferred by at most one control cycle: case (b) never defers a headroom-decreasing reading, and case (a) never defers two cycles in a row.
- [ ] Neither case applies to the grid supply ceiling clamp (C4), which always uses this control cycle's own raw readings.
- [ ] When net import would exceed the effective peak limit minus the safety margin, the charger current is reduced — within the same control cycle in which the accepted household baseline shows it — to the highest whole ampere that keeps net import at or below that target.
...
- [ ] Deadline urgency engages on the first control cycle at which the time needed to reach the active SOC limit at the [escalated maximum permitted rate](system-overview.md#ubiquitous-language) — the rate that would be available *with* urgency's own levers applied — plus a [deadline urgency margin](system-overview.md#ubiquitous-language) of 25% of that time, no longer fits in the time remaining until the deadline. Equivalently: the required current exceeds that rate divided by 1.25. So a charge needing 8 h at the escalated rate engages with about 10 h left, and one needing 2 h engages with about 2½ h left. The occurrence being judged when this fires becomes the [pursued occurrence](system-overview.md#ubiquitous-language), and urgency is in effect for exactly as long as there is one.
- [ ] The escalated maximum permitted rate this test is judged against is computed from a **smoothed** household baseline, not the instantaneous reading. It is a forecast of what urgency could sustain over the remaining window, so a single cycle of household load must not decide it: without smoothing, one kettle can shrink the rate far enough to engage urgency, and — under an `Auto` baseline of `Off`, which can never hand back — hold the maximum peak for the rest of the session off a transient. This is the same split the system already draws elsewhere: the peak-protection clamp (R3) and the grid supply ceiling (C4) read **raw** values so a real breach cannot persist for a smoothing window, while decisions that must not chase transients read smoothed ones (R10). Urgency's own delivery stays clamped on raw readings; only this forecast is smoothed.
- [ ] Every bound of that rate which depends on a household reading is fitted to that same smoothed baseline: both the peak headroom R3 would leave and the headroom the grid supply ceiling (C4) leaves. A bound fitted to a single instant defeats a forecast whichever bound it is. The maximum charging current (C1) depends on no reading and is unaffected, and the peak headroom is a bound at all only where the peak clamp is composed — so not while the CapTar capability is absent (R18), nor under `Power`'s own peak-protection opt-out (R17). This concerns only the headroom those limits leave the forecast, never the clamps themselves, which keep reading raw as the criterion above states.
- [ ] The smoothed baseline that forecast is fitted to is the negation of R10's smoothed [solar surplus](system-overview.md#ubiquitous-language): the mean, over the samples R10's window has admitted, of each control cycle's net import minus that same cycle's charger power. It carries **no** R3 deferral. R3's two cases exist to keep a per-instant clamp from acting on the system's own actuation (case (a)) or on a single optimistic headroom increase (case (b)), and their per-case one-cycle bound is written for that clamp's safety rather than for a forecast. R10's window keeps case (a) out of the forecast: pairing each sample with its own cycle's charger power leaves none of the charger's earlier draw in the mean, and R10's first criterion leaves out the sample taken on the cycle after the charger current changes, which a draw or power reading still lagging that change would spoil, except in the cases that criterion admits it anyway — among them the first set after a restart or reload, which has no earlier set to differ from. It dilutes case (b) to one of the samples the window holds, which is all a forecast needs. So, at a window above 1 and within one unbroken spell of R10's steady inputs in which the net import reading, like the charger's draw and its power reading, reflects each change of charger current within one control cycle, on every control cycle at which every sample the window holds was taken within that spell — none of them taken before the control cycle following the system's second charger-current set since the last restart or reload, nor on a cycle R10 admitted only because it never leaves out two in a row — this smoothed baseline is the spell's own household baseline. The escalated maximum permitted rate then does not depend on the charger current the system sets, so no charger current it sets, on its own, engages urgency or makes the deadline unreachable.
- [ ] Urgency is judged against what escalation could deliver, never against the current the active mode happens to be requesting on this cycle. A mode requesting 0 A therefore does not, on its own, make a deadline urgent while ample time remains — the system waits, and escalates only when the deadline is genuinely at risk. This is what keeps the raise below an exception (C3) rather than the normal state of any evening on which the active mode is idle.
- [ ] Once engaged, the pursued occurrence is held until one of these releases it: the [baseline mode](system-overview.md#ubiquitous-language)'s own desired current reaches the required current *on a cycle at which the engage condition above does not itself hold* (the ordinary policy will now meet the deadline unaided — e.g. the low tariff has opened and `Auto`'s own overnight row would charge anyway); state of charge reaches the active SOC limit; the car is disconnected; the departure deadline resolves to "no deadline" (a later occurrence resolving that way never ends a missed-deadline hold, which is anchored to the occurrence already pursued — see the hold below); the deadline capability becomes absent (R18); or the backstop below fires. The engage condition takes precedence where both hold at once, which it can, since a mode's desired current is what it asks for rather than what can be delivered; without that precedence the two would alternate cycle by cycle. The comparison is against the baseline mode — the one that would be active absent the escalation — never the escalated mode itself, whose request is always the maximum and would clear urgency the instant it engaged. Urgency is **not** re-derived from the engage condition each cycle: charging at the escalated rate closes the gap faster than the deadline approaches, so re-deriving it would revert urgency on the cycle after it engaged and cycle the charger on and off.
- [ ] While urgency is in effect, the system raises the effective peak limit it is willing to create, up to the configured maximum peak (default 4 kW) — accepting a higher monthly peak demand. This is the only lever available under the `Manual` profile: it never raises what the active mode itself requests, so meeting the deadline depends on whether that mode's own request, once unclamped, is enough. When the CapTar capability is absent (R18) there is no peak clamp for the raise to widen (R3), so this lever is a no-op and `Manual` has none — the active mode's own request already stands unclamped by R3, bounded only by C4.
...
**Priority:** Should
**What:** When the `Auto` profile is active, the home-day flag is set for the [reserved day](system-overview.md#ubiquitous-language), the solar forecast for the reserved day is high enough, and no departure deadline is resolved for the reserved day, `Auto` caps the overnight active SOC limit so the reserved day's solar energy can be used instead, and does not itself opportunistically top up from the grid overnight. A departure deadline resolved for the reserved day takes priority over the cap (R14), and so does a missed-deadline hold still in force from a deadline already elapsed (R5) — these two preconditions are what makes the cap and deadline urgency mutually exclusive. This is `Auto`'s own coordination decision (R16) — it does not apply under `Manual`, and it is not a rule the modes `Auto` selects (e.g. `Captar`, R4) enforce themselves; they simply charge to whichever active SOC limit is currently resolved (R7).

**Acceptance criteria:**

- [ ] The cap activates only under the `Auto` profile, and only when the home-day flag is set for the [reserved day](system-overview.md#ubiquitous-language) (R13), the [solar forecast](system-overview.md#ubiquitous-language) yield for the reserved day exceeds a configurable threshold (default 12 kWh), the departure-deadline resolution (R14), evaluated for the reserved day, resolves to "no deadline", and no missed-deadline hold (R5) is in effect — both deadline conditions being trivially satisfied while the deadline capability is absent (R18), since no deadline is ever resolved then and no hold can arise.
- [ ] The cap's three date-bearing conditions — the home-day flag, the solar forecast and the departure-deadline resolution — all read the reserved day: tomorrow's date until midnight, and the date that has just begun from midnight until the sun comes up. The date they read does not change at midnight. While a same-day forecast is mapped, the date rolling over therefore never engages or lifts the cap by itself, and a difference between the two forecast readings is a forecast change like any other; while none is mapped, the cap lifts at midnight (next criterion).
- [ ] The solar forecast for the reserved day is read until midnight from a next-day forecast sensor, mapped whenever the solar capability is present (R18), and from midnight until the sun comes up from a same-day forecast sensor, an optional mapping that is absent by default, so an installation configured before it existed loads and runs without the configuration flow being run again and takes that default (NF12). While the same-day sensor is not mapped, the forecast condition does not hold from midnight, so the cap lifts at midnight — the deliberate fallback. While the reading due is unavailable, the forecast condition does not hold either. Neither sensor is a required role in C5's sense. Until the same-day source's first refresh after midnight, which depends on the provider, its reading may still refer to the day just ended; that window is accepted.
- [ ] While active, the overnight active SOC limit resolves to a configurable value (default 60%) while the sun is down (R7).
- [ ] While active, `Auto` does not select a mode for the sake of opportunistic overnight grid top-up (Auto mode-selection's *Overnight top-up* row, `resolution-rules.md`).
...
- [ ] A departure deadline resolved for the reserved day (R14), or a missed-deadline hold in effect (R5), each takes priority over the cap: the cap does not activate, and if it was already active when either appears, it lifts on the next control cycle. These two preconditions are what "the cap and deadline urgency are mutually exclusive" means. A deadline in force for any other date is not excluded, since it is not competing for the reserve: before midnight, one resolved for today and still ahead of now; from midnight, one resolved for the day after the reserved day.
- [ ] When the sun comes up, `Auto` is no longer active, the home-day flag is no longer set for the reserved day, the solar forecast for the reserved day no longer exceeds its threshold, a departure deadline becomes resolved for the reserved day, or a missed-deadline hold begins, the cap lifts and the active SOC limit resolves normally.
...
- [ ] Net grid power and charger power are sampled together once per control cycle, each pair giving that cycle's solar surplus sample, and every charging-rate decision that reads solar surplus reads the mean of the most recent *N* samples the window has admitted (configurable, default 4 — i.e. `N × control interval` in real time). A cycle's sample is not admitted when the charger current the system set on the previous control cycle (`ChargerCurrentSet`) differed from the one it set before that; the mean of the samples already admitted then stands for that cycle. This never applies on two consecutive control cycles, nor while the window holds no sample yet, nor at a window of 1, which admits every cycle's sample.
...
- [ ] Peak-protection decisions (R3) are exempt from this smoothing window and use raw, unsmoothed readings. The exemption is from *smoothing* only: it does not mean R3 acts on this control cycle's reading unconditionally. R3 applies its own deferrals to the [household baseline](system-overview.md#ubiquitous-language) it solves around, stated in R3 and authoritative there; they are not this window, neither of them can defer a breaching increase by more than one control cycle, and neither is ever substituted for this window in either direction. The sample this window does not admit (the first criterion) is likewise not one of R3's deferrals: each is decided on its own, so neither one counts toward, or rules out, the other.
- [ ] Under steady inputs the charger set-point settles and then holds, in every mode. *Steady inputs* means the household's own consumption, solar production, the supply voltage, the low-tariff flag, the effective peak limit, the active mode and every configured value stay unchanged, every required role (C5) stays available, no restart or reload intervenes (NF14), and the charger's draw, and its power reading, reflect the current it was last set to within one control cycle. Surplus hovering from one cycle to the next, as R2's `round to nearest` midpoint allows, is a changing input and outside this criterion. From the (N + 3)th control cycle after the last change in any of them — N being this smoothing window, so the 7th at the default of 4 — the charger current is the same on every control cycle for as long as they stay unchanged. Only three things may still change it: a hold, peak-breach grace period, cooldown or restart debounce elapsing (R1, R2, R3, R11); the active SOC limit changing or being reached (R6–R9) or deadline urgency engaging or ending (R5), as the car charges and time passes; and the grid supply ceiling clamp (C4) while it binds, which acts on each cycle's own raw readings (R3).
...
- [ ] A cooldown, once started, always runs to completion and is not shortened by a change in conditions — **including a switch of the active mode** — save for exactly three events that end it early: a disconnect, which ends the connection it protected, so the next connection starts with no cooldown running; and a [restart of Home Assistant or a reload of the system](system-overview.md#ubiquitous-language) (the criterion below). A cooldown is scoped to the stop that started it, not to the mode that happened to be active at the time: it keeps charging from starting again in whichever mode is active when it otherwise would, for the duration fixed at the moment charging stopped (the stopping mode's own cooldown period, not the incoming mode's). The protection this requirement exists for is a property of the charger and the car, not of the system's internal mode bookkeeping — charging starting again one control cycle after a stop is equally hard on the car whether the same mode or a different one asks for it — so a mode switch must never be a way out of a cooldown. This matters most under `Auto`, where a mode switch is a routine system-initiated event (deadline-urgency escalation and revert, `resolution-rules.md`) rather than a user action, and would otherwise let a household hovering near the urgency threshold bypass the guarantee entirely. The deliberate trade-off: an urgency escalation (R5) may have to wait out the remainder of a running cooldown — a bounded delay to a best-effort guarantee — rather than this Must-priority hardware protection being defeated. A cooldown that has already elapsed is spent; a later mode switch does not revive it.
- [ ] A [restart of Home Assistant or a reload of the system](system-overview.md#ubiquitous-language) — a reload being how a change saved through the [configuration flow](system-overview.md#ubiquitous-language) takes effect (NF11; ADR-0005, ADR-0008) — ends every running post-surplus hold, peak-breach grace period, cooldown and restart debounce: none carries over, and none resumes or starts over (NF14); a condition that persists past either is a new one, timed from then. The first control cycle after either judges the active mode's own conditions afresh, as on a fresh connection: charging starts as soon as the mode's start condition holds, with no cooldown or restart debounce left to wait out; where it does not hold, the charger is at 0 A from that cycle, without first holding at the minimum charging current. A peak breach that persists is a new one: its grace period is timed from that cycle (R3). The trade-off is deliberate: both events are rare, and a reload is the household reconfiguring the system, whereas the protection this requirement gives is against the system's own normal operation cycling the charger (ADR-0008).
- [ ] In `Solar` and `SolarOnly`, once the has-charged flag is set for the current connection, a start-threshold crossing (from below to at/above the threshold) while dwelling in `Idle` must hold continuously for a configurable restart debounce period (default 1 minute, shared by both modes) before charging actually starts — a single-cycle blip while waiting in `Idle` does not restart charging only to immediately need to stop again. This only gates a *crossing*: if the start threshold is already met at the moment the System enters `Idle` — whether `Idle` was reached because a cooldown elapsed or because the stop at the active SOC limit ended (R7) — there is no crossing to debounce and charging starts immediately, with no additional wait. Before the has-charged flag is first set — the connection's very first start — `Idle` starts charging as soon as the start threshold is met, with no debounce either. `Captar` and `Power` have no restart debounce, since their own start conditions do not depend on a fluctuating sensor reading.
- [ ] The has-charged flag is set the first time a solar mode actually starts charging on the current connection, and is cleared only on disconnect or a restart or reload (NF14) — not by a mode switch, a cooldown elapsing, or reaching the active SOC limit.
- [ ] The charger current is only ever 0 A or at least the minimum charging current, never in between (per C1).
- [ ] Switching the active mode resets the hold and restart-debounce timers so the incoming mode starts fresh against its own thresholds. A hold reset may change *when* a stop happens — switching to a mode with a shorter hold can bring a stop forward — and a debounce reset can only delay a start, never advance one; but neither reset ever shortens the interval this requirement actually protects, the gap between a stop and the restart that follows it, because that gap is governed entirely by the cooldown the stop starts, which a mode switch never resets (the cooldown criterion above). The has-charged flag is likewise unaffected by a mode switch, since it is scoped to the connection, not the active mode.
...
- [ ] The reminder is sent only while the notifications capability is present **and** its own [per-notification enable toggle](system-overview.md#ubiquitous-language) (`plug_in_reminder_enabled`, default on) is on (R18). When that toggle is off, no reminder is sent even though the capability is present and a notification target is mapped. The reminder-due condition above is still evaluated and still exposed as a readout, and the once-per-departure-window rule below still runs while the toggle is off, so turning it back on mid-window releases no suppressed reminder for that window.
- [ ] No further reminder is sent for the same departure window unless the charger is connected and then disconnected again, or a restart or reload of the system intervenes (NF14).
...
- [ ] The evening home-day prompt — the mechanism by which the system asks for the flag rather than waiting to be told — is a notification like R5's notice and R12's reminder, and carries the same kind of [per-notification enable toggle](system-overview.md#ubiquitous-language): it is sent only while the notifications capability is present **and** its own toggle (`evening_prompt_enabled`, default on) is on (R18). The two readings of that toggle coincide, since the prompt *is* the notification: turning it off stops the prompt being sent and, by the same act, withdraws it as a mechanism for setting the flag. Nothing else is withdrawn — every other configured mechanism (an external calendar or presence source, NF3) still sets the flag, and the system's own manual home-day input is always available, so at least one mechanism always remains. Turning the toggle off is therefore never an error state: it only means the system stops asking, and a day nobody sets the flag for is treated as not a home day (below).
...
- [ ] A home-day flag set through the system's own mechanisms — its manual home-day input or the evening prompt — is set for one calendar date, tomorrow's at the moment it is set, and applies to that date alone, until that date ends at midnight. For the whole of that date, while the deadline capability is present (R18), R14 resolves its departure time with the home-day override wherever that override is the one R14's priority order selects — the external departure-time sensor and the public-holiday override both come before it — so the departure-deadline resolution moves past the home day exactly as it would past any other date and never falls back to that date's day-of-week default. No separate reset is needed. The manual home-day input shows and sets tomorrow's flag; a flag already applying to today is fixed once its date has begun.
...
- [ ] The same per-date resolution, evaluated for the [reserved day](system-overview.md#ubiquitous-language) (its day-of-week, holiday status, and home-day flag), feeds one of the solar-reserve cap's two deadline preconditions (R9): a deadline resolved for the reserved day takes priority over the cap, as does a missed-deadline hold (R5, the cap's other deadline precondition, which this resolution does not feed). That precondition is always evaluated for the reserved day, independently of which date the next-occurrence resolution above selects.
...
- [ ] Under `Manual`: mode selection is a pass-through of the user's own selection (no automatic mode changes, NF1); SOC-limit coordination never engages — no solar step-up (R8), and the solar-reserve cap never applies, so the overnight active SOC limit is never capped (R9); no mode-escalation levers are available (the R5 peak-limit raise remains available, as it does under every profile — though when the CapTar capability is absent (R18) there is no peak clamp for the raise to widen (R3), so that lever is a no-op and `Manual` has none at all).
...
- [ ] Under `Auto`, mode selection chooses `Captar` for cost-efficient overnight grid top-up only while the low-tariff flag is active (Auto mode-selection's *Overnight top-up* row, `resolution-rules.md`); the low-tariff preference belongs to this selection, not to `Captar` mode itself (R4) — a manually selected `Captar` session charges regardless of tariff.
...
- [ ] A configurable option determines whether `Power` mode respects CapTar peak protection: when enabled (default), net import stays at or below the effective peak limit minus the safety margin (R3); when disabled, charging may breach the CapTar peak but is still bounded by the grid supply ceiling (C4). The option only ever has an effect while the CapTar capability is present (R18): when it is absent there is no clamp to respect or disable, and `Power` is bounded by C4 alone whatever the option says. Whether the option is presented at all — like the four peak-protection thresholds it sits with — is governed by R18 AC5, which gates the peak-protection fields of the installation flow (R20) on the same capability; this criterion governs only the option's runtime behaviour.
...
- [ ] `Power` runs without a state-of-charge reading. A control cycle on which state of charge is unavailable while `Power` is active is not a [fault](system-overview.md#ubiquitous-language) — C5's role table requires that reading only in `Solar`, `SolarOnly` and `Captar` — and sets the charger current to the Power target current (AC1), subject to the peak-protection clamp where it applies (AC2, R3), the grid supply ceiling (C4), C1 and any running cooldown (R11). On such a cycle the system has no reading to judge the active SOC limit (the criterion above) by, so stopping at it on that cycle is left to the vehicle's own charge limit, which the system keeps in step with the active SOC limit where the vehicle exposes a settable one (R6).
...
**Priority:** Should
**What:** The available charging modes and the behaviours that depend on them adapt to the hardware, the billing arrangement, and the deadline policy the installation actually has, declared as configurable capabilities, so the system remains fully usable, under both `Manual` and `Auto`, on an installation without a solar array, without capacity-tariff billing, with no interest in departure deadlines at all, or with no wish to be contacted by notification at all. Without the solar capability alone, a grid mode (`Captar`) is still reachable under `Auto`, since the CapTar capability is unaffected. Without the CapTar capability, `Auto`'s deadline-urgency escalation (R5) falls back to `Power` instead of `Captar` (R16) — a deliberate, deadline-only exception to `Power` otherwise never being Auto-selected, and one with more headroom than it would have on a CapTar installation, since no peak-protection clamp (R3) bounds it there — while `Auto`'s opportunistic overnight top-up (R16, Auto mode-selection's *Overnight top-up* row) simply does not occur, since there is no deadline forcing it; this also applies when both capabilities are absent. The CapTar capability's absence also carries a Must-level consequence under the `Manual` profile: with no peak clamp (R3) for it to widen, R5's effective-peak-limit raise — `Manual`'s only deadline-urgency lever — becomes a no-op, so on a non-CapTar installation `Manual` has no lever at all and meeting a deadline depends entirely on what the active mode already requests. The solar, CapTar, and deadline capabilities each default to *present*, because each records a fact about the installation that is already true of it; the requirements they gate — including the Must ones (R5, R14) — therefore hold in full on a default installation, save for the notifications that R5, R12, and R13 cannot deliver on one (below). The notifications capability is the one deliberate exception and defaults to *absent*: sending notifications is not an installation fact but a preference for the system to contact the household unprompted, something a household opts into rather than out of, so a household that never answers the question is left un-notified. Its only Must-requirement consequence is R5's unreachable-deadline notice; at Could level, R12's plug-in reminder and R13's evening home-day prompt are equally undelivered on a default installation. R5's charging levers (the peak-limit raise and `Auto`'s escalation) are unaffected, but a default installation has no notification target mapped, so none of those three notifications can be delivered until the household declares the capability present. Declaring it present is necessary but not sufficient for any one of them: each notification also carries its own [per-notification enable toggle](system-overview.md#ubiquitous-language) (below). Beyond that, a capability only ever subtracts behaviour the household has declared it does not have or does not want.
...
- [ ] When the CapTar capability is absent, the `Captar` mode is not offered for manual selection; the `Solar` (subject to the solar capability), `SolarOnly` (subject to the solar capability), `Power`, and `Off` modes remain available for manual selection. `Auto` never chooses `Captar` for opportunistic overnight top-up in this case (R16's *Overnight top-up* row simply does not occur), but does select `Power` — otherwise never an `Auto`-chosen mode — as a best-effort deadline-urgency escalation (R5, R16). The peak-protection clamp itself (R3) does **not run at all** when this capability is absent — R3's own precondition criterion is authoritative — so a non-CapTar installation's net import is bounded only by the grid supply ceiling (C4), typically a far higher ceiling than R3's own thresholds would impose. Its configurability falls away with it: the maximum peak, safety margin, peak floor, and peak-breach grace period, together with the `Power`-mode peak-protection option (R17 AC2, which governs that option's own runtime behaviour) and the optional [external monthly-peak reading](system-overview.md#ubiquitous-language) mapping (R3, absent by default), are presented by the installation flow (R20) only while the CapTar capability is present, and a non-CapTar installation has nothing left for the four thresholds and that option to tune or switch, nor any clamp operand for the mapping to raise. This resolves the tension [UC12](use-cases/UC12-configure-installation-through-guided-flow.md)'s step model surfaced: these thresholds belong with the billing arrangement that motivates tuning them, while the protection of the grid connection that is genuinely ungated is the grid supply ceiling clamp (C4), whose fields the flow presents regardless of any capability declaration.
...
- [ ] When the deadline capability is absent, both of the solar-reserve overnight cap's deadline preconditions (R9) — no departure deadline resolved for the reserved day, and no missed-deadline hold in effect — are always satisfied, so the cap depends only on its remaining conditions. The EV battery capacity (R15) still resolves as configured, but has no remaining effect on charging behaviour, since it feeds only the deadline calculation.
...
- [ ] A capability change never overrides the user's own choice to enable or disable one of the system's entities. An entity the user has disabled stays disabled, and one the user has enabled while a capability that gates it is absent stays enabled — through every later capability change, restart and reload — until the user changes it again. Only an entity whose enabled state the user has left alone follows the capabilities: disabled while a capability that gates it is absent, and back to its default once that capability is present (ADR-0028).
...
- [ ] Every entity classified as runtime configuration in `entity-catalog.md` (`config`-role, or a `state`-role entity the user sets directly, e.g. the active mode selector) is both visible and settable from the dashboard, except one the user has disabled, which holds no value to show or set (R18).
...
- [ ] No entity classified as install-time configuration is presented on the dashboard; install-time configuration is set only through the integration's configuration flow. Each value set through the configuration flow that `entity-catalog.md` lists with a mirror is also available, read-only, as a diagnostic reading, disabled until the user enables it (ADR-0031): a readout of the value, never a second place to set it, and not on the dashboard either.
- [ ] The dashboard is there as soon as the integration is set up, listed in Home Assistant's sidebar, with no step taken by the user to create or add it.
- [ ] The user cannot edit the dashboard; its content is the system's alone, and every restart and reload rebuilds it from the installation's current configuration, so a change saved through the configuration flow — a capability or an adapter-role mapping — shows on it once the reload that saving causes has run (ADR-0022). What happens when the dashboard cannot be provided is NF13's.
...
- [ ] Net grid import is sampled once per control cycle and averaged over a rolling 15-minute window; that 15-minute average, not the per-cycle reading, is the quantity the monthly peak is taken over. This window length is fixed by CapTar's own billing definition and is not a configurable parameter (unlike R10's charging-rate smoothing window). It is separate from, and longer than, that charging-rate smoothing window (R10, default 4 samples): the two are maintained independently and neither's value is substituted for the other's.
...
- [ ] The tracked value is expressed in kilowatts and survives a restart or a reload of the system (NF14): the peak already recorded for the month in progress is not lost and does not begin again from 0 kW. The 15-minute window itself is not preserved and rebuilds from the readings that follow, so the first averages after one reflect only samples taken since.
...
- [ ] The home-day flag survives a restart and a reload still bound to the date it was set for (R13), however long the system was stopped.
- [ ] What the configuration flow captures (R18, NF3) — adapter-role mappings, capability declarations, thresholds and defaults — is unaffected by either; a reload is how a change to it takes effect.
...
The home-day flag is kept because losing it mid-evening would lift the solar-reserve cap (R9) and drop the home-day departure override (R14) the household had asked for; binding it to its date, rather than to when it was set, keeps it from carrying into a day it was not set for.
