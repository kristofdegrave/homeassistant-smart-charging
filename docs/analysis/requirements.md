...
- [ ] The [household baseline](system-overview.md#ubiquitous-language) this check solves around is this control cycle's own reading, except in exactly two bounded cases, in which the most recently accepted reading stands instead: (a) for one control cycle, when the system changed the commanded charger current on the cycle the reading was taken — the reading then partly measures the system's own actuation, seen through two adapter roles with differing refresh rates, rather than the household; (b) for up to a configurable number of consecutive control cycles (default 2), when the reading would *increase* available headroom, until it has held for that many cycles. Neither case ever defers beyond those bounds, and neither applies to the grid supply ceiling (C4), which always uses this control cycle's own raw readings.
...
- [ ] A household increase large enough to breach that target is never deferred by more than one control cycle: case (b) above defers only readings that increase headroom, which are by definition not breaches, and case (a) lasts a single cycle.
...
- [ ] Peak-protection decisions (R3) are exempt from this smoothing window and use raw, unsmoothed readings. The exemption is from *smoothing* only: it does not mean R3 acts on this control cycle's reading unconditionally. R3 applies its own two bounded deferrals to the [household baseline](system-overview.md#ubiquitous-language) it solves around, stated in R3 and authoritative there; they are not this window, are far shorter than it, and are never substituted for it in either direction.
