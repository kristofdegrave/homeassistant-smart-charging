...
- [ ] The cap's three date-bearing conditions — the home-day flag, the solar forecast and the departure-deadline resolution — all read the reserved day: tomorrow's date until midnight, and the date that has just begun from midnight until the sun comes up. The date they read does not change at midnight, so the date rolling over never engages or lifts the cap by itself; a difference between the two forecast readings, or the forecast condition lapsing at midnight while no same-day forecast is mapped (next criterion), is a forecast change like any other.
- [ ] The solar forecast for the reserved day is read until midnight from a next-day forecast sensor, mapped whenever the solar capability is present (R18), and from midnight until the sun comes up from a same-day forecast sensor, an optional mapping that is absent by default so an installation configured before it existed keeps its behaviour after an upgrade (NF12). While the same-day sensor is not mapped, the forecast condition does not hold from midnight, so the cap lifts at midnight — the deliberate fallback. While the reading due is unavailable, the forecast condition does not hold either. Neither sensor is a required role in C5's sense. Until the same-day source's first refresh after midnight (up to an hour), its reading may still refer to the day just ended; that window is accepted.
