...
- [ ] This check uses raw (unsmoothed) sensor readings and never the smoothed net grid power of R10, so that a breach cannot persist for the duration of a smoothing window.
...
- [ ] Once charging has stopped at the active SOC limit, it stays stopped until the active SOC limit rises, a state-of-charge reading is below the active SOC limit, or the car is unplugged and replugged — or a restart or a reload clears the stop (NF14). A lowered limit never ends it; a raised one ends it even when the state of charge, unread on that cycle, is above the new limit. On a cycle with a state-of-charge reading, charging does not resume above the active SOC limit.
