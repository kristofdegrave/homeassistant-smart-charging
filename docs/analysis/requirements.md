...
- [ ] The solar forecast for the reserved day is read from two configured forecast sensors (NF3): a next-day forecast until midnight, and a same-day forecast from midnight until the sun comes up. Both must be mapped whenever the solar capability is present (R18), and neither is a required role in C5's sense: while the reading due is unmapped or unavailable, the forecast condition does not hold.
