...
- [ ] The has-charged flag is set the first time a solar mode actually starts charging on the current connection, and is cleared only on disconnect, a restart or a reload (NF14) — not by a mode switch, a cooldown elapsing, or reaching the active SOC limit.
...
- [ ] `Power` runs without a state-of-charge reading. A control cycle on which state of charge is unavailable while `Power` is active is not a [fault](system-overview.md#ubiquitous-language) — C5's role table requires that reading only in `Solar`, `SolarOnly` and `Captar` — and sets the charger current exactly as the criteria above other than the stop at the active SOC limit would with the reading present. That stop cannot be judged without the reading: on such a cycle it is left to the vehicle's own charge limit, which the system keeps in step with the active SOC limit where the vehicle exposes a settable one (R6); a vehicle that exposes none is not stopped at the limit on that cycle.
