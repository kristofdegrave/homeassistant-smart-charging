...
- [ ] On the lowest-specification hardware Home Assistant officially supports as an installation target (as listed at the time of testing, and again when the integration version under test is published), the mean processing time of one control cycle over any 200 consecutive cycles — excluding time spent waiting for a device or entity to respond — is at most 1 % of the configured control interval (NF11): 100 ms at the default 10 s, 50 ms at the 5 s minimum.
