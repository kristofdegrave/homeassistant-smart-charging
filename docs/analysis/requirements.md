...
- [ ] Every bound of that rate which depends on a household reading is fitted to that same smoothed baseline: both the peak headroom R3 would leave — wherever the peak clamp is composed at all, so not while the CapTar capability is absent (R18) nor under `Power`'s own peak-protection opt-out (R17) — and the headroom the grid supply ceiling (C4) leaves, since a bound fitted to a single instant defeats a forecast whichever bound it is; the maximum charging current (C1) depends on no reading and is unaffected. This concerns only the headroom those limits leave the forecast, never the clamps themselves, which keep reading raw as the criterion above states. The smoothed baseline the forecast is fitted to carries **no** R3 deferral: those two cases exist to keep a per-instant clamp from acting on the system's own actuation or on a single optimistic headroom increase — both of which the smoothing window already suppresses over its own length — and their one-cycle bound is written for that clamp's safety rather than for a forecast.
