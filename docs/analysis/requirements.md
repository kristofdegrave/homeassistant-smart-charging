...
- [ ] That notification is sent **exactly once per occasion**: the user receives one notification on the occasion the deadline becomes unreachable, and no further notification for as long as it stays unreachable — however many control cycles that spans. The notification re-arms the moment the condition clears (the required current falls back within the maximum permitted rate, the car disconnects, state of charge becomes unavailable, the deadline capability becomes absent (R18), or a missed-deadline hold clears), so a later, distinct occasion on which the deadline becomes unreachable again sends a further notification — within the same run of the system, without an intervening restart or reload.
