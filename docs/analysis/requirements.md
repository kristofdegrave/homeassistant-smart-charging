...
- [ ] This requirement applies only while the CapTar capability is present (R18). When it is absent, no peak-protection clamp ever engages: net grid import is bounded only by the grid supply ceiling (C4), and every criterion below is vacuous — except the five [household baseline](system-overview.md#ubiquitous-language) acceptance criteria below, which hold regardless of capability. That resolution runs every control cycle (`control-cycle.md` step 1), not inside the clamp this requirement gates, because readouts gated on other capabilities consume its result — `sensor.smart_charging_solar_surplus_w` on a solar installation without CapTar being the case that forces it.
...
- [ ] The [household baseline](system-overview.md#ubiquitous-language) this check solves around is this control cycle's own reading, except in exactly two cases, in which the most recently accepted reading stands instead: (a) the charger current the System set on the previous control cycle (`ChargerCurrentSet`) differed from the one it set before that, so the reading partly measures the System's own actuation rather than the household — and never on two consecutive cycles, case (a) not applying at all on a control cycle immediately following one it deferred; (b) the reading would *increase* available headroom and has not yet done so on 2 consecutive control cycles.
- [ ] Case (b) holds a headroom-increasing reading indefinitely while it keeps failing to persist for 2 consecutive cycles. That is deliberate — the standing value is the more conservative of the two — and is the only unbounded deferral either case permits.
- [ ] A reading that does go on to persist is accepted after at most 3 consecutive control cycles of deferral: case (a)'s single cycle, then case (b)'s two. The two cases compose only in that order and only for a reading that would increase available headroom; case (a) acting alone may defer a headroom-decreasing reading, for exactly one control cycle.
