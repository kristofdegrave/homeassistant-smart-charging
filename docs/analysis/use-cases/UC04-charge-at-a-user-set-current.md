...
- **Extended by [UC05](UC05-guarantee-ready-by-departure.md)** when the departure deadline is at risk. R5 is a cross-cutting override that may raise the requested current above the configured Power target current — up to the maximum charging current — to the lowest rate that still meets the deadline, and, while `sc_power_respect_peak` is on, may also raise the effective peak limit up to the [maximum peak](../system-overview.md#ubiquitous-language) for the extra headroom the raised current needs; deadline urgency never raises the active SOC limit (R7). When peak protection is off, `Power` is already unconstrained by the effective peak limit, so only R5's current increase (not its peak-raising) has further effect. `Power` is never an `Auto`-selected escalation target (Auto mode-selection selects `Captar` for that, `resolution-rules.md`); this extension applies only to a manually selected `Power` session. The deadline logic lives in UC05, not here.
