...
grid-supply-ceiling clamp (C4) and the C1 floor/cap always apply regardless of that option, and
...
stateDiagram-v2
    ...
    note right of Charging
        ...
        (default): R3 peak clamp fits it (raw) to
        the peak headroom — net import ≤ effective
        peak limit − safety margin. Off: R3 is
        skipped entirely, only the C4 grid-ceiling
        clamp applies. Either way, floor = minimum
        current, cap = maximum current (C1); clamps
        only reduce the request, never raise it
        above the configured target. Tariff- and
        solar-independent.
    end note
```
...
- **May be superseded by the coordinator's deadline-urgency response** (`resolution-rules.md`, `control-cycle.md` step 5) when the departure deadline is at risk — this mode's own set-point rule above is never itself modified (NF2). The coordinator may raise the requested current above the configured Power target current — up to the maximum charging current — to the lowest rate that still meets the deadline, and, while `sc_power_respect_peak` is on, may also raise the effective peak limit up to the [maximum peak](../system-overview.md#ubiquitous-language) for the extra headroom the raised current needs; deadline urgency never raises the active SOC limit (R7). When peak protection is off, `Power` is already unconstrained by the effective peak limit, so only the current increase (not the peak-raising) has further effect. `Power` is never an `Auto`-selected escalation target (Auto mode-selection selects `Captar` for that, `resolution-rules.md`), so this response only ever applies to a manually selected `Power` session. See [UC05](UC05-guarantee-ready-by-departure.md) for the goal this serves; the response mechanism lives in `resolution-rules.md`, not here.
