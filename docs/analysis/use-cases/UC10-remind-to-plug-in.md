...
**Plug-in reminder suppressed by its own enable toggle.**
...
When the plug-in reminder's [per-notification enable toggle](../system-overview.md#ubiquitous-language) (`plug_in_reminder_enabled`) is off, even though the notifications capability is present and a notification target is mapped
Then the System sends no reminder — the driver's goal is not met by this use-case, by the driver's own configuration.
...
And the once-per-departure-window rule still runs: the departure window is consumed exactly as if the reminder had been delivered, so turning the toggle back on part-way through that window releases no suppressed reminder for it. The next reminder the driver can receive is the one for a re-armed window (3a or 3b).
...
- **Sent** — the current departure window's one reminder has been used up; the System sends no further reminder while in this state, even though the preconditions may continue to hold every cycle. The window is consumed whether the reminder was actually delivered or was suppressed by the enable toggle being off — which is why turning the toggle back on mid-window releases nothing.
...
- **R12** — Plug-in reminder notification (all four acceptance criteria: AC1, the single notification within the configured lead time of the next departure time; AC2, delivery gated conjunctively on the notifications capability and the reminder's own enable toggle, with the reminder-due readout and the once-per-window rule still running while the toggle is off; AC3, the connect/disconnect de-dup rule for the same departure window; and AC4, no reminder while already connected or already at/above the active SOC limit).
