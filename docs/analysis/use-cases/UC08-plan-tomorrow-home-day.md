...
This is deliberate and applies to every precondition, not only the notification gating: no precondition turning true later the same evening releases a prompt for it (alternate flows 1a and 1b, and the suppression exception flow, all read this way, consistent with **Not sent** being terminal for the evening in the State model).
...
- **Midnight is this use-case's answer deadline.** This use-case never lets a pending prompt survive past midnight (Main success scenario step 3, Exception flows), so a "yes" or "no" always lands on the same home day it was asked about, and no shorter answer window is applied to the prompt here. `entity-catalog.md` carries a `prompt_timeout_h` row (default 2 h, attributed to R13) that this use-case does not read, and R18 AC10 and the [notifications capability](../system-overview.md#ubiquitous-language) glossary entry both name "the prompt timeout" as part of the notification-configuration surface the capability gates; R13 itself states no acceptance criterion for such a timeout. Reconciling that row and those citations with this use-case's midnight deadline is a requirements-level change to R13 and `entity-catalog.md`, out of this use-case's scope. How R9's overnight solar-reserve window ("while the sun is down") relates to the flag's set-vs-reset moment at midnight remains `resolution-rules.md`'s concern, not this use-case's.
