...
R3's own grace period (default 2 minutes, held at the minimum charging current before the stop) already gives `Captar` the same abrupt-stop protection that the solar modes' post-surplus hold gives them (R1, R2, generalized by R11) — no separate hold is needed here, since the peak clamp's own grace-period mechanism plays that role.
