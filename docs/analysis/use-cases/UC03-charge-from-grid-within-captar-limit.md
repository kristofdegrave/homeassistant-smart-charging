...
- **May be superseded by the coordinator's deadline-urgency response** (`resolution-rules.md`, `control-cycle.md` step 5) when the departure deadline is at risk — this mode's own set-point rule above is never itself modified (NF2). R5 permits high-tariff grid charging and raises the effective peak limit ceiling up to the [maximum peak](../system-overview.md#ubiquitous-language); deadline urgency never raises the active SOC limit (R7), so it still cannot charge beyond a solar-reserve cap `Auto` has applied. Under the `Auto` profile this response is carried by escalating to `Captar` (Auto mode-selection row 2, `resolution-rules.md`) — since `Captar`'s own set-point rule already requests the maximum charging current, this realization delivers up to the [maximum permitted rate](../system-overview.md#ubiquitous-language) rather than the exact lowest rate that closes the gap; a manually selected `Captar` session gets that exact lowest-sufficient-rate realization instead (`resolution-rules.md`'s `Manual` row). See [UC05](UC05-guarantee-ready-by-departure.md) for the goal this serves; the response mechanism lives in `resolution-rules.md`, not here.
