...
- The car is connected at home ([charger status](../system-overview.md#ubiquitous-language) is `connected` or `charging`), state of charge is below the [active SOC limit](../system-overview.md#ubiquitous-language) (resolved per `resolution-rules.md`), and the dispatched mode has computed its own desired current for this cycle (`control-cycle.md`, step 4).
- A [departure deadline](../system-overview.md#ubiquitous-language) is resolved for today — not "no deadline" (`resolution-rules.md`).

## Trigger

A [control cycle](../system-overview.md#ubiquitous-language)'s required-current computation (`resolution-rules.md`) exceeds the dispatched mode's own desired current for this cycle — [deadline urgency](../system-overview.md#ubiquitous-language) (R5).
...
2. **When** the control cycle's required-current computation (`resolution-rules.md`) exceeds that desired current, **then** the System is in deadline urgency (R5).
...
- **Normal** — the required current is at or below the dispatched mode's own desired current; the response does not raise the current, and the effective peak limit resolves normally (`min(monthly peak demand, maximum peak)`).
- **Urgent** — the required current exceeds the dispatched mode's own desired current but is at or below the maximum permitted rate; the response applies (3a/3b) and the effective peak limit is raised to the maximum peak. Under `Manual` (3b), the delivered current is exactly the required current — the lowest rate that closes the gap. Under `Auto`'s `Captar` escalation (3a), the delivered current is `Captar`'s own maximum-current request as clamped to the maximum permitted rate, which can exceed the required current.
...
| --- | --- | --- |
| Normal | Dispatched mode's own desired current, unmodified | required current > desired current, ≤ maximum permitted rate → Urgent · required current > maximum permitted rate → Unreachable |
| Urgent | Required current (`Manual`, 3b) or `Captar`'s maximum-current request clamped to the maximum permitted rate (`Auto`, 3a) — either way ≤ maximum permitted rate; effective peak limit raised to the maximum peak | required current ≤ desired current → Normal (revert) · required current > maximum permitted rate → Unreachable |
| Unreachable | Maximum permitted rate; user notified | required current ≤ maximum permitted rate → Urgent · required current ≤ desired current → Normal |

## Domain events produced

- `DeadlineUrgencyEngaged` — the required current now exceeds the dispatched mode's own desired current; the response (Urgent) takes effect (Normal → Urgent). Emitted by the coordinator, `control-cycle.md` step 5.
- `DeadlineUrgencyReverted` — the dispatched mode's own desired current now meets or exceeds the required current; the response lifts (Urgent/Unreachable → Normal). Emitted by the coordinator, `control-cycle.md` step 9.
- `DeadlineUnreachableNotified` — the required current exceeds the maximum permitted rate; the System delivered the maximum permitted rate and notified the user (Normal/Urgent → Unreachable, or re-fires while remaining in Unreachable). Emitted by the coordinator, `control-cycle.md` step 9.
...
stateDiagram-v2
    [*] --> Normal
    Normal --> Urgent: required current > desired current,<br/>≤ maximum permitted rate
    Normal --> Unreachable: required current ><br/>maximum permitted rate
    Urgent --> Normal: required current ≤<br/>desired current (revert)
    ...
    Unreachable --> Normal: required current ≤<br/>desired current
