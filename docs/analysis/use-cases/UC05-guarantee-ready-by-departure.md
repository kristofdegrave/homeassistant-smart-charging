...
- **Normal** — the required current is at or below the baseline mode's own desired current; the effective peak limit resolves normally (`min(max(monthly peak demand, peak floor), maximum peak)`), and, under `Auto`, mode-selection is unaffected by urgency.
