...
- While deadline urgency holds, the delivered charger current is at least the baseline mode's own desired current and, whenever achievable, at least the required current computed for that cycle — bounded above by the maximum permitted rate (the peak clamp's output under the raised ceiling, itself bounded by C1 and C4); high-tariff charging is permitted for as long as urgency holds.
