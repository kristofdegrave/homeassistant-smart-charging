...
5. **Fix, comment, resolve.** Per finding addressed: fix it, reply on that thread describing
   what was done, then resolve the thread (`resolve-review-thread` has the GraphQL mechanic —
   no REST endpoint exists for this). Resolve only what was actually fixed; leave
