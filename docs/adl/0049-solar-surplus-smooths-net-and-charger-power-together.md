...
- **Which form a step consumes is ADR-0006's, not R10's.**
  ...
  smoothing window. It kept with [ADR-0006](0006-coordinator-and-data-flow.md) the question of
  which form, raw or smoothed, a step consumes: a change there needs an ADR, which may narrow
  one clause as ADR-0036 itself did. Both records
...
- **The evidence is the author's simulation and the HA-harness regressions.** At every window
  ...
  none. The regressions pin windows 4 and 6. At a window of 1 nothing is frozen and the three
  options coincide; no result is recorded for that size with the lag. The freeze's share of A's
  result was not measured apart from the per-sample subtraction.
...
in a row: ADR-0039's rule, with the one-in-a-row cap R3 gives its own deferral. At a window of 1
nothing is frozen.
...
- Pro: it matches the operand that R1, R2, UC01 and UC02's start and stop conditions already
  name, *smoothed solar surplus*.
- Con: step 6 consumes a smoothed form of `charger_w` for the first time, so a lagging charger
  reading enters the window too, and a sample taken with one stays for N cycles. The option
  therefore carries ADR-0039's freeze.
...
against B's and C's first Cons). It takes nothing from R3 or C4.
...
- Easier: above a window of 1 the solar path meets R10's steady-input criterion, so its tests
  need not pin a window of 1 to see a set-point hold.
...
- Follow-up, requirements: a `requirement` task rewords R10's What and its window criterion, the
  ...
  surplus is smoothed from the household baseline and to admit the freeze. Until then the
...
- Out of scope: R5's escalated-rate forecast. R5 fits it to a smoothed, undeferred household
  ...
  record's Context is evidence against that premise for the solar surplus. Which form R5 reads,
  and whether its premise holds, is left to a separate `requirement` task under R5.
...
`rg -n -i 'smoothed_net_w|smooth_net_power|smoothed channel|raw .?charger_w|from net grid power alone|only reading (it|R10) smooths|smooth(ed)? .?net_w|smooth\w*.{0,40}.charger_w.|.charger_w..{0,40}smooth|smoothed (solar )?surplus|smooth\w* (the )?net (grid )?(power|import)|smoothed net|one reading R10 smooths|net grid power is sampled|smoothed value|smoothed\W{0,4}(undeferred )?(household )?baseline|term smoothed|_net_window|_clear_baseline_deferral' custom_components/ tests/ docs/ .claude/ .github/ CLAUDE.md`

— 181 hits, 26 of them in this record. It is wide enough because it is keyed on:
...
| `custom_components/smart_charging/coordinator.py:637` | Smooths `net_w` alone for step 6 | Smooth `net_w − charger_w`, with the freeze |
...
| `tests/test_solar_end_to_end.py:113` | Pins a window of 1: the net-only mean blends in stale readings | The pin may go |
| `docs/analysis/requirements.md:176` | R10: net grid power is the only reading smoothed | The requirements follow-up |
| `docs/analysis/requirements.md:180` | R10's window criterion: net grid power is averaged | The requirements follow-up |
| `docs/analysis/system-overview.md:59` | Overview: the loop smooths net grid power | The requirements follow-up |
| `docs/analysis/system-overview.md:127` | `coordinator`: the same | The requirements follow-up |
| `docs/analysis/system-overview.md:129` | `control cycle`: the same | The requirements follow-up |
...
| `docs/analysis/entity-catalog.md:130` | The solar set-point converges the smoothed net value | The requirements follow-up |
...
| `docs/analysis/control-cycle.md:149` | Step 6 passes smoothed `net_w` | The requirements follow-up |
...
- Eight that hold under either form: `requirements.md:50` (R3 reads raw) and `:505`,
  ...
  `coordinator_cycle.py:79` and `coordinator.py:222`.
- `system-design.md:867` and the ADL's ADR-0036 row, both still true.

Out of scope:
- Seven in `coordinator_cycle.py`, `peak_demand_tracker.py` and their tests keep R21's window.
- The 12 on R5's forecast, three each in `requirements.md` (:90–92), `system-overview.md`,
