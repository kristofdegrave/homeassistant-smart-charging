...
- **R10 requires the set-point to settle under steady inputs**, in every mode, by the (N + 3)th
  ...
  the charger power reading lags one cycle.
- **Which form a step consumes is ADR-0006's, not R10's.**
  ...
  smoothing window, and kept with [ADR-0006](0006-coordinator-and-data-flow.md) which form a
  step consumes: that needs an ADR, which may narrow one clause as ADR-0036 did. Both state that
  charger power is used raw, "not an operand of solar surplus via the smoothed channel".
- **ADR-0039's rejection of a charger window does not carry over.**
  ...
  for R3's clamp because R3 is exempt from smoothing and a window slows a clamp. Step 6 is
...
- Pro: it keeps the raw charger operand and R10's net-only rule.
...
### Option D — Option A without the deferral
...
- the displayed `solar_surplus_w`: R3's accepted baseline, negated and floored at 0;
- ADR-0036's split: R10 owns which readings have a window, ADR-0006 which form a step reads.

The window's deferral applies ADR-0039's rule to a second reading, reusing the coordinator's
command-changed signal; its flag, separate from R3's, travels with the window.
...
| `tests/test_coordinator.py:1041` | Comment: a net-only window | Recompute for the joint one |
| `tests/test_coordinator.py:1043` | Same | Same |
...
| `docs/analysis/requirements.md:176` | R10: net grid power is the only reading smoothed | Requirements |
| `docs/analysis/requirements.md:180` | R10's window criterion: net grid power is averaged | Requirements |
| `docs/analysis/system-overview.md:59` | Overview: the loop smooths net grid power | Requirements |
| `docs/analysis/system-overview.md:127` | `coordinator`: the same | Requirements |
| `docs/analysis/system-overview.md:129` | `control cycle`: the same | Requirements |
| `docs/analysis/system-overview.md:163` | `smoothed value`: a mean of `net_w` only | Requirements |
| `docs/analysis/entity-catalog.md:127` | `net_power`: the one reading R10 smooths | Requirements |
...
| `docs/design/system-design.md:165` | Signal-Conditioning row: smoothed `net_w` | Design |
