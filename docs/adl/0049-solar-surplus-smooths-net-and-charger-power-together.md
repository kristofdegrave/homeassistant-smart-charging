...
decided on smoothing net import and charger power together, per sample, so that R10's
steady-input criterion holds at every window size, accepting that step 6 now consumes a smoothed
form of charger power, so a lagging charger reading has to be kept out of the window.
...
- **The evidence is the author's simulation and the HA-harness regressions.** Option A settles
  in simulation at every window size, with and without the one-cycle lag. The regressions pin
  windows 4 and 6 only. The result recorded for Option B, that it settles at no size, cannot be
  literally true at a window of 1 without lag, where B reduces to the status quo and settles. It
  is read here as true of every window above 1.
...
in a row, which is ADR-0039's rule and cap.

- Pro: each sample's own charger draw cancels, so the window holds no past actuation. It settles in
  simulation at every window size, with and without the lag.
- Pro: it gives the analysis layer what it already names. R1, R2, UC01 and UC02 set the rate
  from *smoothed solar surplus*, not from a smoothed net import.
...
- Pro: it keeps ADR-0036's raw charger operand and R10's "net grid power is the only reading it
  smooths". Only R10's rule for filling the window changes, with no ADR.
...
**Option A.** It is the only option that settles (Option A's first Pro against B's and C's Cons).
...
and its one-in-a-row flag travels with the window the coordinator already threads. That flag is
separate from R3's deferral flag, because each defers a different value.
...
- Follow-up, requirements: a `requirement` task rewords R10's What, its window criterion, the
  glossary's `smoothed value` and `control-cycle.md`'s steps 2 and 6 to say the solar surplus is
  smoothed from the household baseline and to admit the freeze. Until then the analysis layer
  describes Option C.
...
`rg -n -i 'smoothed_net_w|smooth_net_power|smoothed channel|raw .?charger_w|from net grid power alone|only reading (it|R10) smooths|smooth(ed)? .?net_w|smooth\w*.{0,40}.charger_w.|.charger_w..{0,40}smooth|smoothed (solar )?surplus' custom_components/ tests/ docs/ .claude/ .github/ CLAUDE.md`

— 117 hits, 16 of them in this record. It is wide enough because it is keyed on the net-import
mean step 6 reads today and its function, on every wording of the raw-charger clause and of R10's net-only rule, and on
*smoothed surplus*, the consumers' own name for step 6's operand. The dot-directories are named,
...
43 other hits conform: `signal_conditioning.py:16` and the seven in `test_signal_conditioning.py`
(the net-only primitive stays, and the joint window reuses it); the 34 in `requirements.md`
(six), `system-overview.md:173`, `resolution-rules.md:454`, UC01 (12), UC02 (13) and
`.github/create-uc-issues.sh:42`, which already set the rate from *smoothed surplus*; and
`system-design.md:867`, ADR-0036's design row, still true. Out of scope: the seven hits in
`coordinator_cycle.py:34` and `:129`, `peak_demand_tracker.py` and their tests keep R21's own
net-power window; the six in ADR-0006 and ADR-0036 are immutable, and `0006…:91` and `0036…:193`
are the clause this record narrows; the ten in `docs/plans/`, the 17 in `docs/archive/` and
`project-plan.md:407` record what was planned or built at their date; the 16 in this record itself.
