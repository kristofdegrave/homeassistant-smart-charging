...
- **ADR-0025 is superseded in full.** Its Status becomes `Superseded by ADR-0027` and its ADL row is
  ...
  written, per ADR-0001's immutability rule. No other *ADR* cites ADR-0025, but the guided
  config-flow implementation spec does, and makes "ADR-0025 reaching Accepted" its slice gate,
  barring any task from being committed against a superseded ADR-0025 — so merging this change
  puts that spec's own gate condition on a superseded record. Re-pointing that gate, and that
  spec's ADR-0025 citations, at ADR-0027 is a small follow-up update the spec owes once this ADR
  is Accepted; it is named here, not attempted here. Where this
