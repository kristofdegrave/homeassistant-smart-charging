...
In the context of R5's escalated-rate forecast needing a smoothed household baseline for both of
its household-dependent bounds, facing a forecast that would move on the system's own charger
steps, we decided on R10's admitted joint mean, negated, so that no charger step the system makes
moves the forecast, with or without a lagging charger reading, accepting that a household change
landing on the same cycle as a charger step reaches the forecast one cycle later.

## Context

- **R5 requires the forecast smoothed, and names its baseline.** Both bounds that depend on a
  household reading, the peak headroom R3 would leave and the C4 ceiling headroom, are fitted to
  one smoothed [household baseline](../analysis/system-overview.md#ubiquitous-language) without
  R3's deferrals. R5 names it as the negation of R10's smoothed solar surplus, over the samples
  R10 admits.
...
negated. ADR-0006, ADR-0036 and ADR-0049 keep Status `Accepted`, unedited; this record answers
...
- the forecast's freedom from R3's deferrals, R5's; ADR-0039's deferral stays R3's alone;
...
`rg -n -i 'smoothed_net_w|smoothed_baseline_w|smoothed_household_w|smoothed (household )?baseline|undeferred|used raw|raw .?charger(_w| power)|ceiling_headroom_a\(|peak_headroom_a\(|unadmitted|(escalated|forecast).{0,80}(smooth|raw)|(smooth|raw).{0,80}(escalated|forecast)' custom_components/ tests/ docs/ .claude/ .github/ CLAUDE.md`

— 96 hits, 13 of them in this record. It is wide enough because it is keyed on:
- the raw-charger clause, in each record and ADL row that states it;
- the forecast's two bound calls and their helpers, and the joint window's output;
...
| `custom_components/smart_charging/coordinator.py:1548` | Peak headroom bound on raw `baseline_w` | Same |
...
- The ten other design hits: `system-design.md`'s sequence (four) and `project-plan.md` (six),
  which describe it or record it as not built.
- The 30 in `docs/plans/`, which specify the code follow-up as this option.
- The ADL rows for ADR-0006 and ADR-0036, which name this record.

Out of scope:
- ADR-0006's line 90, ADR-0036's line 193 and ADR-0049's nine are immutable; the first two are
  the clause this record narrows.
