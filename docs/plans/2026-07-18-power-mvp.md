...
**Files:**
- Create: `tests/conftest.py` (one root conftest, path-gated — see below)

The `enable_custom_integrations` fixture pulls in phcc's `hass` fixture, so it must **not**
reach the pure `tests/modes/` and `tests/engines/` logic that ADR-0009 (Option A)
deliberately keeps HA-free — otherwise those tests can't collect wherever phcc can't load
(e.g. native Windows, where phcc imports the Unix-only `fcntl`). A blanket `autouse`
fixture at the root would do exactly that. But scoping the conftest to a single HA-harness
dir (`tests/adapters/`) doesn't work either: the plan's later HA-harness tests are
**root-level files** — `tests/test_config_flow.py`, `tests/test_coordinator.py`,
`tests/test_number.py`, `tests/test_sensor.py`, `tests/test_init.py` (ADR-0009's
Consequences names `tests/test_coordinator.py` at the root) — so a `tests/adapters/`-only
conftest would strand them.

The resolution is one root conftest whose autouse fixture is **gated by path**: it applies
`enable_custom_integrations` to every HA-harness test (adapters plus the root-level
config-flow / coordinator / entity / init tests) and skips it only for the pure-logic dirs
(`tests/modes/`, `tests/engines/`). New HA-harness tests are covered automatically; new
pure-logic dirs are added to `_PURE_DIRS`.
...
- HA-harness side (CI/Linux): `pytest -q` resolves the `enable_custom_integrations`
  fixture with no collection error.
- Pure side (any platform, incl. Windows): `pytest -q -p no:homeassistant tests/modes tests/engines`
  collects cleanly with **no** `fixture 'enable_custom_integrations' not found` error
  (exit 5 "no tests ran" until Phase 1 adds tests).

**Step 3: Commit**

```bash
git add tests/conftest.py
git commit --author="Claude <noreply@anthropic.com>" -m "test: add path-gated root conftest (enable_custom_integrations)
